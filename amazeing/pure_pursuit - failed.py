...
#            print('pose updated')
            ...
            self.yaw = yaw
            # print(yaw)
            # print("why isnt yaw updating")
            # print(self.yaw - self.alpha)
    ...
    def run(self):
        try: 
            ...
            if self.pose != None:
                ...
                dx = lookahead.x - x
                dy = lookahead.y - y
                self.alpha = float(math.atan2(dy, dx) - yaw)
                if self.alpha > np.pi:
                    self.alpha -= 2 * np.pi
